...
class CartService {
    ...
    async createCart(userId: string | null): Promise<ApiResponse<Cart>> {
        ...
        const existing = await this.cartRepo.findOne({ where: { user_id: userId } });
        if (existing) {
            return { status: 400, message: 'Cart already exists', data: existing };
        }

        const newCart: Cart = { id: 'new-id', user_id: userId, items: [], totalPrice: 0 };
    ...
    async addCartItem(userId: string, productId: string, qty: number): Promise<ApiResponse<Cart>> {
...
describe('Cart Module - Integration Tests', () => {
    ...
    beforeEach(async () => {
        jest.clearAllMocks();
        cartService = new CartService(mockCartRepo, mockProductRepo, mockInventoryRepo);
    });
    ...
    describe('Create Cart', () => {
        ...
        it('should create a new cart successfully (Happy Case)', async () => {
            ...
            mockCartRepo.save.mockResolvedValue({ id: 'new-id', user_id: '101', items: [], totalPrice: 0 });
        ...
        it('should handle duplicate cart creation', async () => {
            mockCartRepo.findOne.mockResolvedValue({ id: 'old-id', user_id: '101', items: [], totalPrice: 0 });
        ...
        it('should throw exception for invalid user ID', async () => {
            await expect(cartService.createCart(null)).rejects.toThrow('IllegalArgumentException');
            expect(mockCartRepo.save).not.toHaveBeenCalled();
        });
    });
    ...
    describe('Get Cart By User', () => {
        ...
        it('should return cart items for valid user', async () => {
            const mockItems = [{ product_id: 'P1', qty: 1 }, { product_id: 'P2', qty: 1 }];
            mockCartRepo.findOne.mockResolvedValue({ user_id: 'UserA', items: mockItems });
        ...
        it('should handle large data (100 items) within performance limits', async () => {
            ...
            mockCartRepo.findOne.mockResolvedValue({ user_id: 'UserC', items: largeItems });
    ...
    describe('Add Item To Cart', () => {
        ...
        it('should add new item to cart', async () => {
            ...
            expect(mockCart.items[0]).toEqual(expect.objectContaining({ product_id: '101', qty: 2 }));
        });

        // [Cart-9] IT_CART_Add_Accumulate
        it('should accumulate quantity if item exists', async () => {
            mockInventoryRepo.getStock.mockResolvedValue(100);
            const mockCart = { user_id: 'User1', items: [{ product_id: '101', qty: 2, price: 100 }] };
            mockCartRepo.findOne.mockResolvedValue(mockCart);
        ...
        it('should return error if quantity exceeds stock', async () => {
            ...
            const mockCart = { user_id: 'User1', items: [{ product_id: '101', qty: 8, price: 100 }] };
            mockCartRepo.findOne.mockResolvedValue(mockCart);
