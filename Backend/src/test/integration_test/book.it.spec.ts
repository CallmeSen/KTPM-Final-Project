...
class BookService {
    async create(book: Partial<Book>): Promise<ApiResponse> {
        ...
        if (typeof book.price !== 'number') return { status: 400, message: 'Price must be a number' };
        if (book.price <= 0) return { status: 400, message: 'Price must be greater than 0' };

        const newBook = { ...book, id: 'new_id', created_at: new Date().toISOString() } as Book;
    ...
    async update(id: string, data: any): Promise<ApiResponse> {
        ...
        if (data.price !== undefined && data.price < 0) return { status: 400, message: 'Price must be greater than 0' };
    ...
    async remove(id: string): Promise<ApiResponse> {
        ...
        if (activeLoans > 0) return { status: 409, message: 'Sách đang được mượn/đang có giao dịch, không thể xóa' };
    ...
    async booksByCategory(catName: string): Promise<ApiResponse> {
         ...
         const books = await mockBookRepo.find({ where: { category_name: catName } });
         return { status: 200, data: books };
    }
}
...
describe('Book Module - Integration Tests', () => {
    ...
    describe('Create Book', () => {
        ...
        it('should create book successfully with valid data', async () => {
            const res = await bookService.create({ title: 'Clean Code', price: 300000, author: 'Robert C. Martin' });
        ...
        it('should fail when price is not a number', async () => {
            // @ts-ignore
            const res = await bookService.create({ title: 'Test', price: 'Ba trăm nghìn' });
            expect(res.status).toBe(400);
    ...
    describe('Find One Book', () => {
        ...
        it('should return book details when ID exists', async () => {
            mockBookRepo.findOne.mockResolvedValue({ id: '10', title: 'Existing Book', price: 100, author: 'Author' } as Book);
    ...
    describe('Get Books List', () => {
        ...
        it('should handle concurrent requests', async () => {
            const requests = Array(50).fill(null).map(() => bookService.getBooks(10, 1));
    ...
    describe('Update Book', () => {
        const existingBook = { id: '1', title: 'Sách A', price: 100, author: 'Author A', created_at: '2020-01-01' } as Book;
        ...
        it('should not allow updating immutable fields (ID, created_at)', async () => {
            const res = await bookService.update('1', { id: '999', created_at: '1990-01-01' });
            expect(res.status).toBe(200);
    ...
    describe('Search Book', () => {
        ...
        it('should return results for fuzzy search', async () => {
            mockBookRepo.find.mockResolvedValue([{ title: 'Harry Potter...', id: '1', price: 100, author: 'JK' } as Book]);
        ...
        it('should be case insensitive', async () => {
            mockBookRepo.find.mockResolvedValue([{ title: 'Doraemon', id: '2', price: 50, author: 'F' } as Book]);
    ...
    describe('Remove Book', () => {
        ...
        it('should remove book successfully if no constraints', async () => {
            mockBookRepo.findOne.mockResolvedValue({ id: 'B001', title: 'B', price: 10, author: 'A' } as Book);
        ...
        it('should fail to remove if book has active loans', async () => {
            mockBookRepo.findOne.mockResolvedValue({ id: 'B002', title: 'B', price: 10, author: 'A' } as Book);
    ...
    describe('Find By Category', () => {
        ...
        it('should return books for valid category', async () => {
            mockCategoryRepo.findOne.mockResolvedValue({ id: '101', name: 'Science' });
            mockBookRepo.find.mockResolvedValue(Array(5).fill({ category_id: '101' }));
        ...
        it('should return empty list for empty category', async () => {
            mockCategoryRepo.findOne.mockResolvedValue({ id: '102', name: 'New Genre' });
            mockBookRepo.find.mockResolvedValue([]);
    ...
    describe('Filter Books By Category Name', () => {
        ...
        it('should return books when category has data', async () => {
             mockBookRepo.find.mockResolvedValue([{ title: 'Sci-Fi Book', category_name: 'Science', id: '1', price: 10, author: 'A' } as Book]);
