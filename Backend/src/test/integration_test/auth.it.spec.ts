...
class AuthService {
    ...
    async signIn(email: string, pass: string): Promise<AuthResponse> {
        if (!email || !pass) return { status: 400, message: 'Vui lòng nhập đầy đủ thông tin' };
        
        const user = await mockUserRepo.findByEmail(email);
        if (!user) return { status: 404, message: 'Tài khoản không tồn tại' };
        
        if (user.password !== pass) return { status: 401, message: 'Sai mật khẩu hoặc tài khoản' };
    ...
    async refreshToken(token: string): Promise<AuthResponse> {
        if (token === 'Expired_Token') return { status: 401, message: 'Refresh token has expired' };
        if (token === 'Revoked_Token') return { status: 403, message: 'Forbidden' };
        if (token === 'Fake_Payload') return { status: 400, message: 'Invalid Signature' };
        
        return { status: 200, data: { accessToken: 'new_access', refreshToken: 'new_refresh' } };
    }

    async changePassword(user: User, oldPass: string, newPass: string, confirmPass: string): Promise<AuthResponse> {
        if (newPass !== confirmPass) return { status: 400, message: 'Mật khẩu xác nhận không khớp' };
        if (user.password !== oldPass) return { status: 400, message: 'Mật khẩu hiện tại không chính xác' };
        
        return { status: 200, message: 'Cập nhật thành công' };
    }

    async getGoogleAuthUrl(): Promise<AuthResponse> {
        return { status: 200, data: { url: 'https://accounts.google.com/o/oauth2/v2/auth?client_id=mock_client_id' } };
    }

    async googleCallback(code: string, error?: string): Promise<AuthResponse> {
        if (error === 'access_denied') return { status: 302, message: 'Redirect to Login' }; // 302 Found/Redirect
        if (!code && !error) return { status: 400, message: 'Tham số không hợp lệ' };
        if (code === 'fake_code_expired') return { status: 401, message: 'Xác thực Google thất bại' };
    ...
    async resetPasswordRequest(email: string): Promise<AuthResponse> {
        if (!email.includes('@') || !email.includes('.')) return { status: 400, message: 'Định dạng email không hợp lệ' };
        
        const user = await mockUserRepo.findByEmail(email);
        // Security: Always return success even if user not found to prevent enumeration
        return { status: 200, message: 'Vui lòng kiểm tra email để đặt lại mật khẩu' };
    }
}
...
describe('Auth Module - Integration Tests', () => {
    ...
    describe('Register Flow', () => {
        ...
        it('should register a new user successfully (Happy Case)', async () => {
            ...
            mockUserRepo.create.mockResolvedValue({ email: 'newuser@test.com', id: '1' });

            const res = await authService.register({ email: 'newuser@test.com', password: 'Abc@12345', name: 'New User' });
        ...
        it('should fail when registering with existing email', async () => {
            ...
            const res = await authService.register({ email: 'exist@test.com', password: 'Abc@12345' });
        ...
        it('should fail when email format is invalid or password too short', async () => {
            const resEmail = await authService.register({ email: 'abc', password: '123' });
            expect(resEmail.status).toBe(400);
            expect(resEmail.message).toBe('Email không hợp lệ');

            const resPass = await authService.register({ email: 'valid@test.com', password: '123' });
    ...
    describe('Sign In Flow', () => {
        ...
        it('should sign in successfully with valid credentials', async () => {
            mockUserRepo.findByEmail.mockResolvedValue({ email: 'test@email.com', password: 'Password123' });
    ...
    describe('Change Password Flow', () => {
        ...
        it('should change password successfully', async () => {
            const res = await authService.changePassword(mockUser, 'OldPass123', 'NewPass456', 'NewPass456');
            expect(res.status).toBe(200);
        ...
        it('should fail if old password is wrong', async () => {
            const res = await authService.changePassword(mockUser, 'WrongPass', 'NewPass456', 'NewPass456');
            expect(res.status).toBe(400);
        ...
        it('should fail if confirm password does not match', async () => {
            const res = await authService.changePassword(mockUser, 'OldPass123', 'NewPass456', 'NewPass999');
            expect(res.status).toBe(400);
    ...
    describe('Google Login & Callback Flow', () => {
        ...
        it('should create new user if not exists during google callback', async () => {
            ...
            mockUserRepo.create.mockResolvedValue({ email: 'new_user@gmail.com', id: 'new_id' });

            const res = await authService.googleCallback('valid_code_new_user');
            
            expect(res.status).toBe(200);
            expect(mockUserRepo.create).toHaveBeenCalledWith(expect.objectContaining({ email: 'new_user@gmail.com' }));
        });
    ...
    describe('Reset Password Flow', () => {
        ...
        it('should verify email content (Mocked)', async () => {
             ...
             await mockMailService.send('user@test.com', 'Reset Password Request', 'Link...');
