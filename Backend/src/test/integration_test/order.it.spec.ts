...
class OrderService {
  ...
  async updateOrder(
    ...
    if (
      ...
      return {
        status: 400,
        message: 'Quy trình không hợp lệ (Cần giao hàng trước)',
      };
    }
...
describe('Order Module - Integration Tests', () => {
  ...
  describe('Create Order - Transaction & Concurrency', () => {
    ...
    it('should handle concurrency: one succeeds, one fails', async () => {
      ...
      const res1 = await orderService.createOrder(user1, items, {
        card: 'Valid',
      });
  ...
  describe('Find All Orders', () => {
    ...
    it('should sort by createdAt Ascending', async () => {
      ...
      expect(data[0].createdAt.getTime()).toBeLessThan(
        data[1].createdAt.getTime(),
      );
    });
